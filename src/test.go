...
func ndecided(t *testing.T, pxa []*paxos.Paxos, seq int) int {
  count := 0
  var v interface{}
  for i := 0; i < len(pxa); i++ {
    if pxa[i] != nil {
...
func waitn(t *testing.T, pxa[]*paxos.Paxos, seq int, wanted int) {
...
func waitmajority(t *testing.T, pxa[]*paxos.Paxos, seq int) {
  waitn(t, pxa, seq, (len(pxa) / 2) + 1)
}

func checkmax(t *testing.T, pxa[]*paxos.Paxos, seq int, max int) {
...
func TestForgetMem(t *testing.T) {
  ...
  fmt.Printf("Test: paxos.Paxos frees forgotten instance memory ...\n")
    ...
    for i := 0; i < len(p); i++ {
      for j := 0; j < len(p); j++ {
        ...
        if err != nil {
          ...
          // corresponding paxos.Paxos peer has prematurely quit and
